...
import Data.Bits (Bits (popCount), testBit, xor, (.&.), shiftR)
    ...
    AVar _ vid -> do
      ...
      case bind of
        ...
        Just b  -> case b of
          ...
          Symbolic (TUInt (Just n)) offset -> return $ map (+cOffset) [fromInteger offset.. fromInteger offset+fromInteger n - 1]
    AIndex _ vid e -> do
      e' <- reduceExpr e
      case e' of
        VInt i -> do
...
uopFns uop = case uop of
  ...
  ArcsinOp   -> ( Nothing
                , Nothing
                , Nothing
                , Just $ \f -> VFloat $ asin f )
  ArccosOp   -> ( Nothing
                , Nothing
                , Nothing
                , Just $ \f -> VFloat $ acos f )
  ArctanOp   -> ( Nothing
                , Nothing
                , Nothing
                , Just $ \f -> VFloat $ atan f )
  CeilOp     -> ( Nothing
                , Nothing
                , Just $ \i -> VInt i 
                , Just $ \f -> VInt $ ceiling f )
  FloorOp    -> ( Nothing
                , Nothing
                , Just $ \i -> VInt i 
                , Just $ \f -> VInt $ floor f )
  ...
  SqrtOp     -> ( Nothing
                , Nothing
                , Nothing
                , Just $ \f -> VFloat $ sqrt f )
  NegOp      -> ( Just $ \s -> VPoly $ head (sNot [s])
                , Just $ \t -> VPolyList $ sNot t
                , Nothing
                , Nothing )
  UMinusOp   -> ( Just $ \s -> VPoly $ head (sNot [s])
                , Just $ \t -> VPolyList $ sNeg t
                , Just $ \i -> VInt (-i)
                , Just $ \f -> VFloat (-f) )
  PopcountOp -> ( Nothing
...
reduceUOP uop v = do
  ...
  i <- getIntOfValue v
  case (i, h) of
    (Just i, Just h) -> return $ h i
    _ -> do
      p <- getPolyOfValue v
      case (p, f) of
        (Just p, Just f) -> return $ f p
        _ -> do
  ...
  where
    ...
    z' path expr = case typeof path of
      ...
      TCReg n -> do
        ...
        return $ (Pathsum 0 0 (fromInteger n) 0 0 (setWidth pl (fromInteger n)), pl)
      TUInt (Just n) -> do
        pl <- exprToSBVList expr
        return $ (Pathsum 0 0 (fromInteger n) 0 0 (setWidth pl (fromInteger n)), pl)
